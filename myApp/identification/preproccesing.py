...
def preProccessVideo():

    mp_pose = mp.solutions.pose
    pose = mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, min_tracking_confidence=0.5)

    # Path to the input video
    video_path = 'no/murasifabnormal.mp4'
    ...
    integer_angles = np.floor(curves).astype(int)
    reference_curve = np.mean(curves, axis=0)
    reference_curve =pd.DataFrame(reference_curve)
    total_seconds= valid_frames/fps
    cadence = (total_steps/total_seconds)*60
    ...
    semalarity=0
    # Compare each curve to the reference curve
    for i, curve in enumerate(curves):
        score = similarity_score(curve, reference_curve)
        if score < threshold:
            print(f"Gait cycle {i} is not similar to a normal gait cycle.")
        else:
            print(f"Gait cycle {i} is similar to a normal gait cycle.")
            semalarity +=1
    if semalarity/len(curves) >0.9:
        return 0
    else:
        return 1